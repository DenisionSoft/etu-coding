...
import com.example.misis2.productspecification.ProductSpecification;
import com.example.misis2.productspecification.ProductSpecificationService;
...
class ResponseScreenMediumClassification extends JFrame {
    public ResponseScreenMediumClassification(Integer product_id, Integer pos, Integer part_id, Integer quantity) {
        super("Ответ");
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        setSize(500, 150);
        setLayout(null);
        ...
        String[] colNames = {"product_id", "pos", "part_id", "quantity"};
        Object[][] data = {
                {product_id, pos, part_id, quantity}
        };
...
class InsertScreen extends JFrame {
    ...
    public void dataScreen(String type) {
        ...
        dataScreenFrame.setSize(250, 340);
        ...
        nameLabel.setEnabled(type != "Спецификация");
        dataScreenFrame.add(nameLabel);

        JTextField nameField = new JTextField();
        nameField.setBounds(110, 45, 120, 25);
        nameField.setEnabled(type != "Спецификация");
        ...
        classLabel.setEnabled(type == "Класс изделия" || type == "Изделие" || type == "Спецификация");
        ...
        classSpinner.setEnabled(type == "Класс изделия" || type == "Изделие" || type == "Спецификация");
        ...
        partLabel.setBounds(10, 185, 100, 25);
        partLabel.setEnabled(type == "Спецификация");
        dataScreenFrame.add(partLabel);

        JSpinner partSpinner = new JSpinner(new SpinnerNumberModel(1, 1, null, 1));
        partSpinner.setBounds(110, 185, 120, 25);
        partSpinner.setEnabled(type == "Спецификация");
        dataScreenFrame.add(partSpinner);

        JLabel quantityLabel = new JLabel("Количество");
        quantityLabel.setBounds(10, 220, 100, 25);
        quantityLabel.setEnabled(type == "Спецификация");
        dataScreenFrame.add(quantityLabel);

        JSpinner quantitySpinner = new JSpinner(new SpinnerNumberModel(1, 1, null, 1));
        quantitySpinner.setBounds(110, 220, 120, 25);
        quantitySpinner.setEnabled(type == "Спецификация");
        dataScreenFrame.add(quantitySpinner);

        JButton button = new JButton("Создать");
        button.setBounds(75, 255, 100, 25);
        button.addActionListener(e -> {
            ...
            Integer r_part_id = (Integer) partSpinner.getValue();
            Integer r_quantity = (Integer) quantitySpinner.getValue();
            Integer response;
            switch (type) {
                case "Базовая ЕИ":
                ...
                case "Изделие":
                    ...
                    break;
                case "Спецификация":
                    response = ProductSpecificationService.ins_product_specification(r_parent_id, r_part_id, r_quantity);
                    if (response == 1)
                        new ResponseScreenSmall("Успех", "");
                    else if (response == -1)
                        new ResponseScreenSmall("Ошибка", "Изделия с id " + r_parent_id + " не существует");
                    else if (response == -2)
                        new ResponseScreenSmall("Ошибка", "Детали с id " + r_part_id + " не существует");
                    else if (response == -3)
                        new ResponseScreenSmall("Ошибка", "Циклы недопустимы");
                    else
                        new ResponseScreenSmall("Ошибка", "Спецификация уже существует");
            }
        });
...
class DeleteScreen extends JFrame {
    public DeleteScreen() {
        ...
        comboBox.addItem("Изделие");
        comboBox.addItem("Спецификация");
        comboBox.addItem("Все спецификации продукта");
        comboBox.setSelectedItem("Базовая ЕИ");
        comboBox.setBounds(20, 45, 200, 25);
        add(comboBox);
        ...
        JLabel posLabel = new JLabel("Позиция");
        posLabel.setBounds(30, 115, 60, 25);
        add(posLabel);

        JSpinner posSpinner = new JSpinner(new SpinnerNumberModel(1, 1, null, 1));
        posSpinner.setBounds(150, 115, 60, 25);
        add(posSpinner);

        JButton button = new JButton("Удалить");
        button.setBounds(75, 160, 100, 25);
        button.addActionListener(e -> {
            Integer id = (Integer) idSpinner.getValue();
            Integer pos = (Integer) posSpinner.getValue();
            ...
            switch (comboBox.getSelectedItem().toString()) {
                ...
                case "Изделие":
                    ...
                    break;
                case "Спецификация":
                    response = ProductSpecificationService.del_product_specification(id, pos);
                    if (response == 1)
                        new ResponseScreenSmall("Успех", "");
                    else
                        new ResponseScreenSmall("Ошибка", "Спецификация не существует");
                    break;
                case "Все спецификации продукта":
                    response = ProductSpecificationService.del_product_specifications_by_product(id);
                    if (response == 1)
                        new ResponseScreenSmall("Успех", "");
                    else
                        new ResponseScreenSmall("Ошибка", "Изделие не существует");
...
class UpdateScreen extends JFrame {
    public UpdateScreen() {
        ...
        separator.setBounds(0, 140, 250, 10);
        add(separator);

        JLabel specificationLabel = new JLabel("Для спецификации");
        specificationLabel.setBounds(65, 150, 140, 25);
        add(specificationLabel);

        JLabel prodLabel = new JLabel("Изделие");
        prodLabel.setBounds(30, 170, 60, 25);
        add(prodLabel);

        JSpinner prodSpinner = new JSpinner(new SpinnerNumberModel(1, 1, null, 1));
        prodSpinner.setBounds(30, 200, 60, 25);
        add(prodSpinner);

        JLabel posLabel = new JLabel("Позиция");
        posLabel.setBounds(100, 170, 60, 25);
        add(posLabel);

        JSpinner posSpinner = new JSpinner(new SpinnerNumberModel(1, 1, null, 1));
        posSpinner.setBounds(100, 200, 60, 25);
        add(posSpinner);
        ...
        JSpinner quantitySpinner = new JSpinner(new SpinnerNumberModel(1, 1, null, 1));
        quantitySpinner.setBounds(130, 230, 100, 25);
        add(quantitySpinner);

        JSeparator separator2 = new JSeparator();
        separator2.setBounds(0, 260, 250, 10);
        add(separator2);

        JButton button = new JButton("Задать");
        button.setBounds(65, 270, 100, 25);
        button.addActionListener(e -> {
            Integer id = (Integer) idSpinner.getValue();
            String name = nameField.getText();
            Integer prod = (Integer) prodSpinner.getValue();
            Integer pos = (Integer) posSpinner.getValue();
            ...
            switch (comboBox.getSelectedItem().toString()) {
                ...
                case "Изделие":
                    ...
                    break;
                case "Спецификация":
                    response = ProductSpecificationService.update_product_specification_quantity(id, pos, quantity);
                    if (response == 1)
                        new ResponseScreenSmall("Успех", "");
                    else
                        new ResponseScreenSmall("Ошибка", "Спецификация не существует");
...
class SelectScreen extends JFrame {
    public SelectScreen() {
        ...
        JLabel posLabel = new JLabel("Позиция");
        posLabel.setBounds(20, 115, 60, 25);
        add(posLabel);

        JSpinner posSpinner = new JSpinner(new SpinnerNumberModel(1, 1, null, 1));
        posSpinner.setBounds(90, 115, 120, 25);
        add(posSpinner);
        ...
        button.addActionListener(e -> {
            ...
            switch (comboBox.getSelectedItem().toString()) {
                ...
                case "Класс изделия":
                    ...
                    if (pc.getBaseEI() == null && pc.getName() == "null") {
                        new ResponseScreenSmall("Ошибка", "Класс не существует");
                        break;
                    }
                ...
                case "Родитель изделия":
                    ...
                    break;
                case "Спецификация":
                    ProductSpecification product_specification = ProductSpecificationService.select_product_specification(id, pos);
                    if (product_specification.getIdProduct() == null) {
                        new ResponseScreenSmall("Ошибка", "Спецификация не существует");
                        break;
                    }
                    new ResponseScreenMediumClassification(product_specification.getIdProduct(), product_specification.getPosNum(), product_specification.getIdPart(), product_specification.getQuantity());
            }
        });
        add(button);
...
class SearchScreen extends JFrame {
    public SearchScreen() {
        ...
        comboBox.addItem("Вывести спецификацию");
        comboBox.addItem("Посчитать спецификацию");
        ...
        button.addActionListener(e -> {
            Integer id = (Integer) idSpinner.getValue();
            String[] columns = {"id_class", "name", "base_ei", "parent_id", "id_product", "name_product", "id_class", "id_product", "pos", "id_part", "part_name", "quantity"};
            switch (comboBox.getSelectedItem().toString()) {
                case "Потомки класса":
                ...
                case "Родителей изделия":
                    ...
                    break;
                case "Вывести спецификацию":
                    sortScreen(id, "Вывести спецификацию", copyOfRange(columns, 7, 12));
                    break;
                case "Посчитать спецификацию":
                    sortScreen(id, "Посчитать спецификацию", copyOfRange(columns, 9, 12));
                    break;
            }
        });
        add(button);
    ...
    public void sortScreen(Integer id, String item, String[] columns) {
        ...
        sortScreenFrame.add(amountLabel);

        JSpinner amountSpinner = new JSpinner(new SpinnerNumberModel(1, 1, null, 1));
        amountSpinner.setBounds(120, 80, 100, 25);
        sortScreenFrame.add(amountSpinner);
        ...
        button.addActionListener(e -> {
            ...
            switch (item) {
                ...
                case "Вывести спецификацию": {
                    ArrayList<?> response = ProductSpecificationService.select_product_specifications_with_names(id, column);
                    if (response.size() == 0) {
                        new ResponseScreenSmall("Ошибка", "");
                        break;
                    }
                ...
                case "Посчитать спецификацию": {
                    ArrayList<?> response = ProductSpecificationService.count_product_specifications_unique_with_names(id, amount);
                    if (response.size() == 0) {
                        new ResponseScreenSmall("Ошибка", "");
                        break;
                    }
