...
# 1.1
MeanCI(dataset[, 13], conf.level = 0.9)

# 1.2
murange <- c(0, 10, 50)
popvar <- task3[10, 13]

t.score <- round((mean(dataset[, 13]) - murange) / 
                   (popvar / sqrt(length(dataset[, 13]))), 3)
print(t.score)
pnorm(abs(t.score), lower.tail = FALSE)

t.test(dataset[, 13], mu = murange[1], 
       alternative="two.sided", conf.level = 0.9)
t.test(dataset[, 13], mu = murange[2], 
       alternative="two.sided", conf.level = 0.9)
t.test(dataset[, 13], mu = murange[3], 
       alternative="two.sided", conf.level = 0.9)

# 1.3
vt1 <- EnvStats::varTest(dataset[, 13], alternative="greater", 
                         conf.level=0.9, sigma.squared = 1)
vt2 <- EnvStats::varTest(dataset[, 13], alternative="greater", 
                         conf.level=0.9, sigma.squared = 4)
vt3 <- EnvStats::varTest(dataset[, 13], alternative="greater", 
                         conf.level=0.9, sigma.squared = 9)

vt1p <- vt1$p.value - 0.1
vt2p <- vt2$p.value - 0.1
vt3p <- vt3$p.value - 0.1

# 2.1
monmeans <- c(0, 0, 0)
for (i in (1:3)) {
  monmeans[i] <- task3[8, i+2]
}
monmeans

weights <- c(31/92, 30/92, 31/92)

ast <- weighted.mean(monmeans, weights)

# 2.2
t.test(dataset[, 3], mu = ast, alternative="greater", 
       conf.level = 0.9)
t.test(dataset[, 4], mu = ast, alternative="two.sided", 
       conf.level = 0.9)
t.test(dataset[, 5], mu = ast, alternative="less", 
       conf.level = 0.9)

# 3
newCL <- round((1 - (0.1/3)), digits = 2)
t.test(dataset[, 3], dataset[, 4], 
       var.equal = FALSE, mu = 0, alternative = "two.sided", 
       conf.level = newCL)
t.test(dataset[, 4], dataset[, 5], 
       var.equal = FALSE, mu = 0, alternative = "two.sided", 
       conf.level = newCL)
t.test(dataset[, 3], dataset[, 5], 
       var.equal = FALSE, mu = 0, alternative = "two.sided", 
       conf.level = newCL)